...
- **Banner localization:** treat the official creative as the final composition and preserve its full crop. Translate only meaningful visible copy into concise natural Korean inside the original banner. Use a transparent, feathered Squid-dark subtitle gradient only around the detected copy so the source remains visible; never use a separate footer, solid caption box, blurred patch, or thick text outline. Keep the source line count and rhythm; a short repeated Latin keyword may remain when it carries the original visual. When the creative has no translatable copy, preserve the character and artwork without adding a headline, badge, gradient, footer, CTA, or duplicate logo.
