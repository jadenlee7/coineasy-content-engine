...
- **Banner localization:** treat the official creative as the final composition and preserve its full crop. Translate only meaningful visible copy into concise natural Korean as a nearby in-banner subtitle without covering the source lettering. Keep the subtitle background fully transparent and use only a thin readability outline; never add a separate footer, Squid-colored caption area, gradient, solid box, blurred patch, or thick outline. Keep the source line count and rhythm; a short repeated Latin keyword may remain when it carries the original visual. When the creative has no translatable copy, preserve the character and artwork without adding a headline, badge, footer, CTA, or duplicate logo.
