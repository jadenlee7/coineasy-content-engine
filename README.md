...
OriginTrail media can enter the review-only Batch lane only through an exact private registry match: standalone source proof, source-body hash, raw X media identity, canonical preview hash, and a human-qualified supporting-evidence envelope must all agree. Media is provenance rather than factual evidence, and Studio exposes the limitations and official references read-only. The four generated copy fields and Buzz v1 notification remain unchanged, and every output still requires human double fact-checking. See [`docs/ADR-013-origintrail-reviewed-media-evidence.md`](docs/ADR-013-origintrail-reviewed-media-evidence.md).
