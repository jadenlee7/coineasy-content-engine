...
The first Buzz integration is a separate GET-only OriginTrail shadow projection at `/api/buzz-shadow/origintrail/batch`. It reuses the Batch review RPC but returns only a deterministic event ID, exact agent/workflow identity, review status, model tier, measured cost, official X URL, and an authenticated Studio deep-link path. Draft copy, prompts, provider IDs, token counts, Supabase credentials, and every approve/publish/deploy capability stay behind the existing execution plane. The production reader is protected by its own `BUZZ_SHADOW_ACCESS_TOKEN`. A feature-gated next slice adds a private durable receipt, distinct `BUZZ_DELIVERY_WORKER_TOKEN`, and `/api/buzz-delivery/origintrail` transition endpoint so an isolated one-shot worker can fence the official v0.5.3 CLI before any relay call. It remains disabled and requires explicit `--send-once`; no Buzz message has been sent. See [`docs/BUZZ_ORIGINTRAIL_SHADOW.md`](docs/BUZZ_ORIGINTRAIL_SHADOW.md) and [`docs/ADR-011-origintrail-buzz-durable-delivery.md`](docs/ADR-011-origintrail-buzz-durable-delivery.md).
