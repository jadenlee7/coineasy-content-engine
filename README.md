...
This branch contains the reviewed Supabase schema and team UI, but the migration is intentionally **not applied to a live project**. Configure a dedicated staging project, `STUDIO_ACCESS_TOKEN`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, and `CONTENT_STUDIO_WORKSPACE_ID` before enabling durable tutorial generation. Daily News and Article persistence, scheduled source collection, approval/publishing screens, and the internal Figma plugin are the next delivery phase; the current UI does not claim those are already live.
