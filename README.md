...
The Buzz integration is a separate GET-only OriginTrail shadow projection at `/api/buzz-shadow/origintrail/batch`. It returns deterministic review identity, measured cost, official X and authenticated Studio links, plus only a bounded Korean headline and Telegram summary after the explicit `BUZZ_RESULT_PREVIEW_START_AT` cutover. Long-form body, X copy, prompts, provider IDs, token counts, Supabase credentials, and every approve/publish/deploy capability stay behind the existing execution plane. The production reader uses its own `BUZZ_SHADOW_ACCESS_TOKEN`; a distinct `BUZZ_DELIVERY_WORKER_TOKEN` and durable receipt endpoint fence the official desktop-v0.5.4 CLI before one relay write. The hourly Railway worker reconciles expired leases on every run, and both Netlify Buzz functions use dedicated scoped Supabase role JWTs (`SUPABASE_BUZZ_DELIVERY_KEY` / `SUPABASE_BUZZ_SHADOW_KEY`) as bearer credentials while retaining the project API key. Automatic publication remains off. See [`docs/BUZZ_ORIGINTRAIL_SHADOW.md`](docs/BUZZ_ORIGINTRAIL_SHADOW.md) and [`docs/ADR-011-origintrail-buzz-durable-delivery.md`](docs/ADR-011-origintrail-buzz-durable-delivery.md).
