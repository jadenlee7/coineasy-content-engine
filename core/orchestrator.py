...
def generate_edu_carousel(
    ...
    """
    End-to-end: source content → LLM spec → N PNGs + manifest.
    
    Args:
        client_id: which client (must exist in clients/<client_id>/config.yaml)
        source_content: English source text
        source_type: "tweet" | "blog" | "article"
        source_url: original URL for attribution
        series_number: manual override
        output_dir: where to write PNGs (default: ./output/<client>/<timestamp>)
        mock_llm: skip Anthropic API call, use mock_llm_response
    
    Returns:
        CarouselResult with all paths and metadata
    """
    ...
    for lesson in spec["lessons"]:
        ...
        if not template_file:
            print(f"  ⚠ Unknown layout {layout}, skipping lesson {lesson['lesson_number']}")
            continue
        
        # Inject default icons (LLM doesn't produce them)
        _inject_default_icons(layout, lesson["slots"])
        ...
        try:
            ...
            print(f"  ✓ Lesson {lesson['lesson_number']} ({layout}) → {output_png.name}")
        except Exception as e:
            print(f"  ✗ Lesson {lesson['lesson_number']} failed: {e}")
