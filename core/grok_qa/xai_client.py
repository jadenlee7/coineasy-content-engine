...
def _prompt_text(claim: GrokQaWorkClaim) -> str:
    ...
    return (
        ...
        "verified_official_source_remix, do not treat no_text metadata as proof "
        "that the pixels contain no meaningful English copy. Independently inspect "
        "the banner pixels. Official logos, wordmarks, handles, URLs, and protected "
        "product or platform names may remain, but any meaningful English headline, "
        "caption, or metric label requires brand_check.status=BLOCK, decision=BLOCK, "
        "and next_action=revise_banner. Review rendered/public fields only, and do "
